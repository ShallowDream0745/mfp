...
def get_result(
    ...
    if env_type == "myosuite":
        selected_keys = ["test/reward", "test/avg_success"]
        domain_filter = ["myosuite"]
    elif env_type == "dm_control":
        selected_keys = ["test/reward"]
        domain_filter = ["dog", "humanoid"]
    else:
    ...
    # Process runs
    print(f"Processing runs with env_type={env_type}...")
    process_runs(
        runs,
        selected_keys=selected_keys,
        domain_filter=domain_filter,
        output_dir=Path(base_dir) / env_type / "flowrl",
    )
